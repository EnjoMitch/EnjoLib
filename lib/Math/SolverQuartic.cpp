...
SolverQuartic::Roots SolverQuartic::Run(double a, double b, double c, double d, double e)
{
	...
	if (a == 0)
	{
		throw EnjoLib::ExceptRuntimeError("The coefficient of the power four of x is 0. Please use the utility for a third degree quadratic.");
	}
	if (e == 0)
	{
		throw EnjoLib::ExceptRuntimeError("One root is 0. Now divide through by x and use the utility for a third degree quadratic to solve the resulting equation for the other three roots.");
	}
	if (a != 1)
	{
		b /= a;
